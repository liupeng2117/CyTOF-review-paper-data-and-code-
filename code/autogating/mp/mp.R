...
setwd("D:/research/Liza/cytof/CyTOF existing methods and challenges/programs/autogating/mp")
mp_result<-read.csv("result.csv")
cluster_label<-mp_result$predicted
ref_label<-mp_result$X
library(plyr)
...
library(mclust)
adjustedRandIndex(ref_label[ids],cluster_label[ids]) #0.55
