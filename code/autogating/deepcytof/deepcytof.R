...
setwd("D:/research/Liza/cytof/CyTOF existing methods and challenges/programs/autogating/deepcytof")
deepcytof_result<-read.csv("result.csv")
cluster_label<-deepcytof_result$predicted
ref_label<-deepcytof_result$X
library(plyr)
...
library(mclust)
adjustedRandIndex(ref_label[ids],cluster_label[ids]) #0.9581686
...
for(i in 1:20){
  for(j in 1:nclust){
    a<-which(ref_label==i) #true
    b<-which(cluster_label==j & ref_label!=0) #predicted, cells labeled with 0 are not included in evaluation
